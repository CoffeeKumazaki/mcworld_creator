...
TSUNAMI_PATH = ".//wtr:WaterBody"

def load_polygons(file):

  doc = et.parse(file, None)
  polygons = []
  for obj in doc.iterfind(TSUNAMI_PATH, namespaces=doc.getroot().nsmap):
    rings = []
    wtr_class = obj.find("./wtr:function", namespaces=doc.getroot().nsmap)
    for polygon in obj.iterfind(".//gml:Polygon", namespaces=doc.getroot().nsmap):
      ...
      ## exterior の 高さ成分をwtr_class に置き換える
      exterior[:, 2] = float(wtr_class.text)
